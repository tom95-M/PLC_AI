...
{
    public class MitsubishiPlcCommunicator : IPlcCommunicator
    {
        ...
        public MitsubishiPlcCommunicator(PlcConfig config)
        {
            _config = config;
            if (_config.Port == 102)
            {
        ...
        public async Task<object> ReadAsync(string address, string dataType)
        {
            ...
            {
                EnsureConnected();
                var startAddress = ParseDAddress(address);
        ...
        private static int ParseDAddress(string address)
        {
            ...
            if (!normalized.StartsWith("D") || !int.TryParse(normalized[1..], out var index) || index < 0)
            {
                throw new NotSupportedException($"Only D-register addresses are supported for Mitsubishi (example: D100). Current: {address}");
            }
